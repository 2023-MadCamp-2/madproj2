...
const ContactListPage = ({navigation}) => {
  ...
  useEffect(() => {
      ...
      const fetchContacts = async () => {
        try {
          const response = await fetch('http://172.10.5.132:443/contact', {
            method: 'GET',
            headers: {
  ...
  const handleDeleteContact = async (contact) => {
      try {
        const response = await fetch(`http://172.10.5.132:443/contact/${contact.nickname}`, {
          method: 'DELETE',
        });
        ...
        filteredContacts && (
          ...
          renderItem={({ item }) => (
            <View style={styles.card}>
              ...
              {selectedContact && selectedContact.nickname == item.nickname && (
                <ContactDropdownMenu
